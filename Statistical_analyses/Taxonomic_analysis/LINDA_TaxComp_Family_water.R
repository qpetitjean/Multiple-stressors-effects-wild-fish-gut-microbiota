...
#               at the Family Level in Water Samples using LMM
#
...
# ==============================================================================
# Script Overview:
# This script performs differential abundance analysis at the family level for water samples 
...
# 5. Retrieving OTU taxonomy and aggregating read counts at the family level.
...
#    - Relative abundance plots (SVG format) at the family level.
...
kableExtra::save_kable(modified_html_string, 
                       file.path(DirSave, "Signif_Effects", "Signif_Plots", "Water", "TaxoLindaWater", 
                                 paste0("TaxFamilyLindaWaterTabSignif", ".HTML")))
...
expandedCol<- colorRampPalette(colorsTax)(50)
linda.obj$output[[i]]$colors[which(linda.obj$output[[i]]$VolcanoGroups == "p-adj. & Log2FC")] <- sample(expandedCol, taxL, replace = FALSE)
...
toSave <- file.path(savingDir, "Normalized_Data", "Signif_Effects", "Signif_Plots", "Water", "TaxoLindaWater", "Family_Water_Contam.tif")
...
toSave <- file.path(savingDir, "Normalized_Data", "Signif_Effects", "Signif_Plots", 
                    "Water", "TaxoLindaWater", "EC_Contam_Water_RelAb_Family_TOP.svg")
if(file.exists(toSave)){
  unlink(toSave)
} 
ggplot2::ggsave(
...
# save the plot
toSave <- file.path(savingDir, "Normalized_Data", "Signif_Effects", "Signif_Plots", 
                    "Water", "TaxoLindaWater", "EC_Contam_Water_RelAb_Family_Signif.svg")
if(file.exists(toSave)){
  unlink(toSave)
} 
ggplot2::ggsave(
