...
for (i in names(normalizedDat)) {
  ...
  for (k in distances) {
    ...
    for (l in c("PCOA", "NMDS")) {
        ...
        if (length(list.dirs(file.path(DirSave, "Signif_Effects", "Signif_Plots", "LMM", "BetaDivPlots"))) == 0) {
          dir.create(file.path(DirSave, "Signif_Effects", "Signif_Plots", "LMM", "BetaDivPlots"))
        }
        if (length(list.dirs(file.path(DirSave, "Signif_Effects", "Signif_Plots", "LMM", "BetaDivPlots", i))) == 0) {
          dir.create(file.path(DirSave, "Signif_Effects", "Signif_Plots", "LMM", "BetaDivPlots", i))
        }
        if (length(list.dirs(file.path(DirSave, "Signif_Effects", "Signif_Plots", "LMM", "BetaDivPlots", i, k))) == 0) {
          dir.create(file.path(DirSave, "Signif_Effects", "Signif_Plots", "LMM", "BetaDivPlots", i, k))
        }
        if (length(list.dirs(file.path(DirSave, "Signif_Effects", "Signif_Plots", "LMM", "BetaDivPlots", i, k, l))) == 0) {
          dir.create(file.path(DirSave, "Signif_Effects", "Signif_Plots", "LMM", "BetaDivPlots", i, k, l))
        }
          toSave <- file.path(DirSave, "Signif_Effects", "Signif_Plots", "LMM", "BetaDivPlots", i, k, l, paste0("Mod_Perf_", axe, ".tif" ))
        ...
        toSave <- file.path(DirSave, "Signif_Effects", "Signif_Plots", "LMM", "BetaDivPlots", i, k, l, paste0("Signif-Effects-Plots_", axe, ".tif" ))
        if(file.exists(toSave)){
          unlink(toSave)
        } 
        grDevices::tiff(filename = toSave,
    ...
    for (j in names(PlotList[[i]])) {
      PlotsToSave <-
        ggpubr::ggarrange(plotlist = PlotList[[i]][[j]],
                          ncol = 4,
                          nrow = 3)
      if (length(list.dirs(file.path(DirSave, i))) == 0) {
        dir.create(file.path(DirSave, i))
      }
      if (length(list.dirs(file.path(DirSave, i, j))) == 0) {
        dir.create(file.path(DirSave, i, j))
      }
      toSave <- paste0(file.path(
        DirSave, i, j, paste(h, i, j, "Norm", sep = "_")
      ), ".svg")
      if(file.exists(toSave)){
        unlink(toSave)
      } 
...
saveRDS(DistList, paste0(file.path(DirSave, "DistMatrices"), ".RDS"), compress = TRUE)

# save the BetaDivCoords (PCOA and NMDS) as .RDS
saveRDS(BetaDivCoords, paste0(file.path(DirSave, "BetaDivCoords"), ".RDS"), compress = TRUE)

# save statistical test outputs
saveRDS(LMMRes, file = file.path(DirSave, "Signif_Effects", "LMMRes.rds"), compress = TRUE)

# save Envfit test outputs
saveRDS(EnvFitRes, file = file.path(DirSave, "Signif_Effects", "EnvfitRes.rds"), compress = TRUE)
...
if (length(list.dirs(file.path(DirSave, "Signif_Effects", "LMMBetaDivBestModTab"))) == 0) {
  dir.create(file.path(DirSave, "Signif_Effects", "LMMBetaDivBestModTab"))
} 
...
for(m in distances){
  ...
  kableExtra::save_kable(l[[length(l)]], 
                         file.path(DirSave, "Signif_Effects", "LMMBetaDivBestModTab", 
                                   paste0(paste("LMMBetaDivBestMod", m, Ord, axe, sep = "_"), 
                                          ".HTML")))
}
  }
  }
...
kableExtra::save_kable(TabList[[axis]], 
                       file.path(DirSave, "Signif_Effects", "LMMBetaDivBestModTab", 
                                 paste0(paste("LMMBetaDivBestMod", dataNorm, ordination, axis, sep = "_"), 
                                        ".HTML")))
}
...
toSave <- paste0(file.path(
  DirSave, ordination, "Bray", paste("Contam", ordination, dataNorm, "Bray", sep = "_")
), ".svg")
if(file.exists(toSave)){
  unlink(toSave)
} 
...
# generate the table (here one table per metric - e.g., richness, shannon, ... - including all normalisation methods)
if (length(list.dirs(file.path(DirSave, "Signif_Effects", "EnvFitBetaDivBestModTab"))) == 0) {
  dir.create(file.path(DirSave, "Signif_Effects", "EnvFitBetaDivBestModTab"))
} 
...
for(Ord in c("PCOA", "NMDS")){
    for(m in distances){
      ...
      l[[length(l)]]
      kableExtra::save_kable(l[[length(l)]], 
                             file.path(DirSave, "Signif_Effects", "EnvFitBetaDivBestModTab", 
                                       paste0(paste("EnvFitBetaDivBestModTab", m, Ord, sep = "_"), 
                                              ".HTML")))
    }
}
...
ordination <- "PCOA"
  ...
  kableExtra::save_kable(l[[length(l)]], 
                         file.path(DirSave, "Signif_Effects", "EnvFitBetaDivBestModTab", 
                                   paste0(paste("EnvFitBetaDivBestModTab", dataNorm, ordination, sep = "_"), 
                                          ".HTML")))
